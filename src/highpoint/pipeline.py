...
from highpoint.utils import great_circle_distance_m, meters_to_miles, miles_to_meters
...
def _load_terrain(config: AppConfig) -> Tuple[TerrainGrid, Tuple[float, float], Transformer]:
    ...
    if terrain_cfg.data_path is None:
        ...
        grid = loader.read(bounds=bounds, resolution_scale=terrain_cfg.resolution_scale)
    observer_xy = transformer.transform(
        config.observer.longitude, config.observer.latitude
    )
